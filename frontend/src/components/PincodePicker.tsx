...
export function PincodePicker({ onDone }: { onDone?: () => void }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {gpsState === "denied" && (
        <p className="-mt-2 text-xs text-muted">Location permission denied — search your town or pincode below.</p>
      )}
      {gpsState === "error" && <p className="-mt-2 text-xs text-muted">Couldn&apos;t find that — try searching below.</p>}

      <div className="flex items-center gap-3">
        <div className="h-px flex-1 bg-border" />
        <span className="text-xs font-medium text-muted-soft">OR</span>
        <div className="h-px flex-1 bg-border" />
      </div>
          ...
          {displayResults.map((r) => (
            <li key={`${r.pincode}-${r.locality}`}>
              <button onClick={() => choose(r)} className="flex w-full items-center gap-3 px-4 py-3 text-left text-sm hover:bg-brand-soft/40">
                <MapPin size={16} className="shrink-0 text-muted-soft" />
                <span>
